...
Nodes defined within the context manager are meant to be associated with the
context (DslContext; e.g. ConditionalContext). If with blocks are nested,
multiple context are associated (note that context order matters). Associated
context objects are not public API but are preserved in pipeline IR, later used
for orchestator & input resolution behavior, visualization, etc.
"""
...
# Cannot import base_node module (circular dependency).
BaseNode = Any


def _generate_context_id(self) -> str:
  return f'{self.__class__.__name__}:{len(_registry.all_contexts) + 1}'
...
class DslContext:
  ...
  DslContext is associated with nodes that are defined within the context
  manager ("with" block) that has created the context. If the node is defined
  within the multiple context maangers, all active contexts are associated with
  the node.
  """
  ...
  def ancestors(self) -> Iterable['DslContext']:
    """All ancestor contexts in parent -> child order."""
    if self.parent:
  ...
  def nodes(self) -> List[BaseNode]:
    """All nodes that are associated with this context."""
    return _registry.get_nodes(self)


class _DslContextRegistry(threading.local):
  """Registry for DSL contexts and their nodes when running a pipeline DSL.

  _DslContextRegistry manages the active contexts, their orders, and
  associations between nodes and contexts.
  """
  ...
  def put_node(self, node: BaseNode) -> None:
    """Associates the node to all active contexts."""
    for context in self._active:
      self._nodes_by_context_ids[context.id].append(node)

  def get_nodes(self, context: DslContext) -> List[BaseNode]:
    """Gets all nodes that belongs to the context."""
    return list(self._nodes_by_context_ids[context.id])

  def get_contexts(self, node: BaseNode) -> List[DslContext]:
    """Gets all contexts that the node belongs to."""
...
def put_node(node: BaseNode) -> None:
  """Puts BaseNode to the currently active DSL contexts."""
  _registry.put_node(node)


def get_contexts(node: BaseNode) -> List[DslContext]:
  """Gets all contexts that the node belongs to."""
