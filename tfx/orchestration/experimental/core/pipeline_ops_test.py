...
class PipelineOpsTest(test_utils.TfxTest, parameterized.TestCase):
  ...
  def test_resume_manual_node(self):
    ...
    self.load_proto_from_text(
        os.path.join(
            os.path.dirname(__file__), 'testdata', 'manual_node.pbtxt'),
        pipeline)
    ...
    with self._mlmd_connection as m:
      ...
      with mlmd_state.mlmd_execution_atomic_op(
          mlmd_handle=m,
          execution_id=execution.id) as execution:
      ...
      pipeline_uid = task_lib.PipelineUid.from_pipeline(pipeline)
      node_uid = task_lib.NodeUid(node_id='manual_a', pipeline_uid=pipeline_uid)
